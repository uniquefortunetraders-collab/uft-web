...
export function BlogsManager({ initialPosts, categories }: BlogsManagerProps) {
  ...
  return (
    <div className="space-y-8 max-w-6xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-gray-900">Blogs CMS</h1>
          <p className="text-xs text-gray-500 mt-1">Publish blog posts, market guides, and technology updates.</p>
        </div>
      </div>
      ...
      <div ref={scrollRef}>
        <Card className={`p-6 bg-white border transition-colors ${editingPost ? 'border-[#e6005c] ring-1 ring-[#e6005c]/20' : 'border-gray-200'}`}>
            ...
            <h2 className="text-base font-bold text-gray-900 flex items-center gap-2">
              {editingPost ? (
                <>
                  ...
                  <span>Edit Blog Post: <span className="text-[#e6005c]">{editingPost.title}</span></span>
                </>
              ) : (
                <>
          ...
          <form
            ref={formElementRef}
            key={`blog-form-${editingPost?.id || 'new'}-${formKey}`}
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">
                  Estimated Reading Time (minutes) <span className="text-gray-400 font-normal">(Leave blank to auto-calculate)</span>
                </label>
                <input
            ...
            <ImageUpload
              ...
              label="Blog Featured Cover Image"
              defaultValue={editingPost?.featured_image_url}
              aspectRatio="video"
              helperText="Drag and drop or upload banner image for this blog post."
            />
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Brief summary displayed on blog listing cards..."
                className="w-full px-3 py-2 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#e6005c] focus:outline-none"
              />
            </div>

            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Full Blog Body Content</label>
              <textarea
            ...
            <div className="flex items-center gap-3 pt-2">
              <Button type="submit" variant="primary" disabled={isSubmitting} className="text-xs">
                {isSubmitting ? (editingPost ? 'Updating...' : 'Saving...') : editingPost ? 'Update Blog Post' : 'Save Blog Post'}
              </Button>
              {editingPost && (
      ...
      <Card className="p-6 bg-white border border-gray-200">
        <h2 className="text-base font-bold text-gray-900 mb-4">Published Blogs ({initialPosts?.length || 0})</h2>

        {!initialPosts || initialPosts.length === 0 ? (
          <div className="text-xs text-gray-500 py-6 text-center">No blogs published yet. Write your first post above.</div>
        ) : (
